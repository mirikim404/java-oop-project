...
public class EncounterView extends JPanel {
    ...
    // ✅ 등장 메시지 — 복수 몹 대응
    List<Mob> waveMobs = waveManager.getMobsForWave(wave);
    String msg;
    if (waveMobs.size() > 1) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < waveMobs.size(); i++) {
            if (i > 0) sb.append(" + ");
            sb.append("[ ").append(waveMobs.get(i).getName()).append(" ]");
        }
        sb.append(" 이(가) 나타났다!");
        msg = sb.toString();
    } else {
        msg = "[ " + mob.getName() + " ] 이(가) 나타났다!";
    }

    JLabel msgLabel = new JLabel(msg, SwingConstants.CENTER);
    msgLabel.setFont(new Font("Dialog", Font.BOLD, waveMobs.size() > 1 ? 22 : 28)); // ✅ 글자 수 많으면 폰트 줄임
